...
export type UserWithSwords = Prisma.UserGetPayload<{
	include: {
		swords: true;
	};
}>;
...
export async function totalLuck(user: {
	...
	if (user.currentSword) {
		const sword = user.swords.find((s) => s.id === user.currentSword);
		...
		return Math.floor(
			userLuck * vip * level * weekend * sword.luck * levelLuck,
		);
	}

	return Math.floor(userLuck * vip * level * weekend * levelLuck);
}
...
export async function getProperty(
	...
	const userTotalLuck = await totalLuck({
		...
		vip: user.vip,
		currentSword: user.currentSword,
		swords: user.swords,
	});
...
export async function generateSword(user: UserWithSwords) {
	...
	experience = Math.floor(
		experience * enchantExperience * levelBonus * titleBonus,
	);
